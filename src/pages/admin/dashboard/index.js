...
//   //   { name: 'Jumlah Penduduk', data: [0,0,0,0,0] }
//   // ]);

//   const [educationSeries, setEducationSeries] = useState([
//     { series: [0,0,0,0,0], labels: [
...
//           await Promise.all(pendidikans.slice(0, -1).map(async (pendidikan) => {
//             const { count, error } = await supabase
//               .from('penduduks')
//               .select('*', { count: 'exact' })
...
//           setEducationSeries({
//             series: pendidikans.map(status => pendidikanCounts[status]),
//             labels: pendidikans
//           });
//         } catch (error) {
